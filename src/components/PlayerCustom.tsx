...
import { formatTime } from './icon';
import { getRangeBox } from './icon';
...
export const AudioPlayer: React.FC<AudioInterface> = ({
  ...
  return (
    ...
    >
      ...
      {/* {canPlay && !hasError && (
        <div className="rap-pp-button" onClick={() => togglePlay()} id='buttonToClick'>
        ...
        <div className="rap-pp-button" onClick={() => togglePlayCustom()} id='buttonToClick'>
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="24" viewBox="0 0 18 24" className="rap-pp-icon">
            <path fill={color ?? '#566574'} fillRule="evenodd" d={isPlaying ? iconPaths.pause : iconPaths.play} />
          </svg>
        </div>
      ...
      >
        <source src={trackToPlay} type="audio/mpeg" />
      </audio>
    </div>
  );
};