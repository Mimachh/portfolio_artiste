import React from 'react'
import Link from "next/link"
import { BsArrowRight } from "react-icons/bs"
import { motion } from 'framer-motion'

import { PostData } from './Blog'
import { fadeIn } from '@/utils/variants'

export default function PostList({ posts }: { posts: PostData[] }) {
      const firstThreePosts = posts.slice(0, 3);
      console.log(firstThreePosts)

  return (

    <motion.div 
    variants={fadeIn('up', 0.4)}
    initial='hidden'
    whileInView={'show'}
    viewport={{once: false, amount: 0.3}}   
    className='flex flex-col items-center'>
      <div className='flex flex-col xl:flex-row justify-between gap-12 py-10 xl:pt-16 xl:pb-24
      border-t border-white-10
      '>
        {firstThreePosts.map((post) => {
          //destructure post.
          const {id, date, title, description} = post;
          return <div key={id} className='flex-1'>
            <div className='text-primary/80'>{date}</div>
            <div className='text-cl font-medium mb-4'>{title}</div>
            <p className='text-white/50 mb-6 font-light'>{description}</p>
            <Link href={"#"} className='flex items-center gap-x-2 group'>
              Read more
              <BsArrowRight className='text-xl group-hover:ml-1 transition-all' />
            </Link>
          </div>
        })}
      </div>
      <button className='btn btn-lg btn-accent'>View all posts</button>
    </motion.div>
  )
}
